    ...
    if(plots==TRUE){
      ...
      for (i in 1:length(cdates)){
        ...
        p<-ggplot(plotdf, aes(x = x, y = y)) + 
          ...
          xlab("Year") +
          ylab("Density")+
          ggtitle(paste("Probability of HIV infection at diagnosis of",df[i,2]))+ 
          theme_classic()+
          scale_fill_manual(values=c("#0BB702", "#F8766D"), labels=c(paste(df[i,3],"Yes"),paste(df[i,4],"No")),name="Probs")
